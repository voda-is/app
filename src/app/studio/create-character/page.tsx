...
import { IoSave, IoArrowBack, IoPersonCircle, IoCloudUpload, IoImage } from 'react-icons/io5';
import { useUser, useCreateCharacter, useUpload } from '@/hooks/api';
...
export default function CreateCharacterPage() {
  ...
  const isCreating = isCreatingCharacter || isUploading || isUploadingAvatar || isUploadingBackground;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      createCharacter(characterData, {
        ...
        onError: (createError) => {
          setError(`Failed to create character: ${createError.message}`);
        }
      });
    } catch (err) {
                ...
                <div className="relative w-full aspect-square bg-gray-800 rounded-lg overflow-hidden border-2 border-dashed border-white/20 flex items-center justify-center">
                  {avatarPreview ? (
                    ...
                    <div className="text-center p-4">
                      ...
                      <p className="mt-2 text-sm text-gray-400">No avatar selected</p>
                    </div>
                  )}
